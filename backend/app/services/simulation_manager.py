...
class SimulationManager:
    ...
    def prepare_simulation(
        ...
        try:
            ...
            # FIX: OasisProfileGenerator lacks generate_profiles_from_entities, 
            # refactored to iterate and generate in parallel using ThreadPoolExecutor.
            profiles = []
            
            with ThreadPoolExecutor(max_workers=parallel_profile_count) as executor:
                # Map each entity to a unique user_id starting from 1
                future_to_entity = {
                    executor.submit(
                ...
                for future in as_completed(future_to_entity):
                    ...
                    try:
                        ...
                        profile_progress(len(profiles), total_entities, f"Generated: {entity.name}")
                    except Exception as exc:
                        logger.error(f"Entity profile generation failed for {entity.name}: {exc}")
                        # Robustness: mark as skipped but continue the pipeline
                        profile_progress(len(profiles), total_entities, f"Failed: {entity.name}")

            state.profiles_count = len(profiles)
            
            # 保存Profile文件
            # FIX: generator.save_profiles is also missing. Handled via local helpers.
            if progress_callback:
                progress_callback(
            ...
            if progress_callback:
                progress_callback(
                    ...
                    f"Profile生成完成，共 {len(profiles)} 个",
    ...
    def _save_profiles_to_json(self, profiles: List[OasisAgentProfile], file_path: str, platform: str):
        """保存Profile为JSON格式"""
        data = []
        for p in profiles:
            if platform == "reddit":
    ...
    def _save_profiles_to_csv(self, profiles: List[OasisAgentProfile], file_path: str):
        """保存Profile为CSV格式（Twitter专用）"""
        if not profiles:
            return
            
        # Get headers from the first profile
