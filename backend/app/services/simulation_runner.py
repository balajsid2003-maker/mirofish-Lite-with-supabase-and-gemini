...
# Zep removed — graph memory update is a no-op in MiroFish Lite
class _NoopGraphMemoryManager:
    @staticmethod
    def create_updater(*a, **kw): pass
    @staticmethod
    def stop_updater(*a, **kw): pass
    @staticmethod
    def get_updater(*a, **kw): return None

ZepGraphMemoryManager = _NoopGraphMemoryManager()
...
class SimulationRunner:
    ...
    def start_simulation(
        ...
        if not Config.LLM_API_KEY:
            ...
            raise ValueError("No LLM provider configured. Please set GEMINI_API_KEY in your .env file.")
        ...
        # 启动前清理：删除旧的数据库文件和 IPC 文件以确保环境干净
        try:
            for ext in ["*.db", "*.ipc", "*.lock"]:
                import glob
                for f in glob.glob(os.path.join(sim_dir, ext)):
                    try:
                        os.remove(f)
                        logger.info(f"清理旧模拟文件: {f}")
                    except Exception:
                        pass
            
            # 清理旧的动作日志
            for p in ["twitter", "reddit"]:
                log_f = os.path.join(sim_dir, p, "actions.jsonl")
                if os.path.exists(log_f):
                    try:
                        os.remove(log_f)
                        logger.info(f"清理旧动作日志: {log_f}")
                    except Exception:
                        pass

            # 强制清空旧的 Profile，确保重新生成最新的 schema (user_char 等)
            for p_file in ["twitter_profiles.csv", "reddit_profiles.json"]:
                p_path = os.path.join(sim_dir, p_file)
                if os.path.exists(p_path):
                    try:
                        os.remove(p_path)
                        logger.info(f"清理旧Profile: {p_file}")
                    except Exception:
                        pass
        except Exception as e:
            logger.warning(f"启动前清理失败 (非致命): {e}")
    ...
    def _monitor_simulation(cls, simulation_id: str):
        ...
        # 默认 2 小时超时（7200秒），防止进程死锁或无限挂起
        timeout = 7200 
        last_log_time = start_time
        
        logger.info(f"开始监控模拟进程: {simulation_id}, pid={process.pid}")
        ...
        try:
            while process.poll() is None:  # 进程仍在运行
                ...
                if elapsed > timeout:
                    logger.error(f"模拟超时(>2h): {simulation_id}, 正在强制终止...")
                ...
                # 早期崩溃检测：如果进程在 10 秒内退出，且没有生成动作日志
                # 这通常意味着初始化失败（如数据库锁定、依赖缺失、API错误）
                # (这个逻辑在 while 循环外处理退出码)

                # 每 60 秒记录一次心跳日志
                if time.time() - last_log_time > 60:
                    logger.info(f"模拟监控中: {simulation_id}, 进度: {state.current_round}/{state.total_rounds} 轮")
            ...
            if return_code != 0:
                logger.error(f"模拟进程异常退出: {simulation_id}, pid={process.pid}, exit_code={return_code}")
                state.runner_status = RunnerStatus.FAILED
                
                # 尝试从日志中提取错误原因
                error_msg = f"进程异常退出 (退出码: {return_code})"
                try:
                    log_path = os.path.join(sim_dir, "simulation.log")
                    if os.path.exists(log_path):
                        with open(log_path, 'r', encoding='utf-8') as f:
                            lines = f.readlines()
                            # 查找最近的 Traceback 或 Error
                            relevant_lines = []
                            for line in reversed(lines[-20:]): # 检查最后20行
                                if "Traceback" in line or "Error" in line or "Exception" in line or "KeyError" in line:
                                    relevant_lines = lines[lines.index(line):]
                                    break
                            
                            if not relevant_lines:
                                relevant_lines = lines[-5:] # 如果没找到关键词，取最后5行
                                
                            error_detail = "".join(relevant_lines).strip()
                            if error_detail:
                                error_msg = f"启动失败: {error_detail}"
                except Exception as log_err:
                    logger.warning(f"读取错误日志失败: {log_err}")
                
                state.error = error_msg
                cls._save_run_state(state)
            else:
                logger.info(f"模拟进程自然结束: {simulation_id}, pid={process.pid}")
