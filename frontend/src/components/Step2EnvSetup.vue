...
const isStarting = ref(false)     // 正在启动模拟
...
const handleStartSimulation = async () => {
  ...
  try {
    ...
    if (params.maxRounds) {
      startParams.max_rounds = params.maxRounds
    }
    
    addLog(`发送启动指令 (max_rounds: ${startParams.max_rounds || 'auto'})...`)
    const res = await startSimulation(startParams)
    ...
    if (res.success) {
      ...
      addLog(`✗ 模拟启动失败: ${errorMsg}`)
      alert(`启动失败: ${errorMsg}`)
    }
  } catch (err) {
    const errorMsg = err.message || '网络请求异常'
    addLog(`✗ 启动异常: ${errorMsg}`)
    alert(`启动异常: ${errorMsg}`)
  } finally {
