<template>
  <div class="simulation-panel">
    ...
    <div class="main-content-area" ref="scrollContainer">
      <!-- Error Banner (if any) -->
      <div v-if="runStatus.error" class="error-banner">
        ...
        <div class="error-details">
          <div class="error-title">Simulation Error</div>
          <div class="error-message">{{ runStatus.error }}</div>
        </div>
        <button class="manual-start-btn mini" @click="doStartSimulation">RETRY</button>
      </div>
      ...
      <div class="timeline-feed">
        ...
        <div v-if="allActions.length === 0" class="waiting-state">
          <div v-if="!runStatus.error">
            <div class="pulse-ring"></div>
            <span class="waiting-msg">{{ isStarting ? 'Initializing simulation engine...' : 'Waiting for agent actions...' }}</span>
          </div>
          
          <div v-if="runStatus.error" class="failed-state">
            <div class="failed-icon">!</div>
            <span class="failed-msg">Simulation failed to start correctly. Check monitors below.</span>
          </div>
          
          <!-- Manual Start Fallback -->
          <button 
            v-if="!isStarting && (runStatus.runner_status === 'idle' || runStatus.runner_status === 'failed' || !runStatus.runner_status)" 
            class="manual-start-btn" 
            @click="doStartSimulation"
          >
            {{ runStatus.runner_status === 'failed' ? 'Restart Simulation' : 'Manual Start' }}
          </button>
        </div>
      </div>
    </div>
...
onMounted(() => {
  ...
  if (props.simulationId) {
    // 之前是在此处自动启动，现在改为由 Step 2 直接启动并跳转。
    // 这里只需开启轮询即可。
    phase.value = 1
    startStatusPolling()
    startDetailPolling()
